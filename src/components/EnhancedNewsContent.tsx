...
const EnhancedNewsContent: React.FC<EnhancedNewsContentProps> = ({ content }) => {
  const renderContent = (section: ContentSection, index: number) => {
    switch (section.type) {
      case 'heading':
        return (
          <h2 key={index} className="text-3xl font-bold mb-6 text-[#000f60] border-b-2 border-[#000f60] pb-2">
            {section.content}
          </h2>
        );
      ...
      case 'subheading':
        return (
          <h3 key={index} className="text-2xl font-semibold mb-4 text-[#000f60]">
            {section.content}
          </h3>
        );
        ...
        return (
          <div key={index} className="grid gap-4 mb-8">
            {section.items?.map((item, itemIndex) => {
              ...
              return (
                <Card key={itemIndex} className="border-2 border-[#ff5310]">
                  <CardContent className="p-4 flex items-start space-x-4">
                    <div className="bg-[#ff5310] p-2 rounded-lg">
                      <IconComponent className="h-6 w-6 text-white"/>
                    </div>
                    <p className="text-gray-800 leading-relaxed flex-1 mt-2">{item}</p>
                  </CardContent>
                </Card>
              );
            })}
        ...
        return (
          <div key={index} className="mb-8">
            <ol className="list-none space-y-3">
              {section.items?.map((item, itemIndex) => (
                <li key={itemIndex} className="text-gray-800 leading-relaxed text-sm bg-gray-50 p-3 rounded border-l-4 border-[#ff5310]">
                  {item}
                </li>
              ))}
            </ol>
        ...
        return (
          <div key={index} className="grid md:grid-cols-3 gap-6 mb-8">
            {section.statsData?.map((stat, statIndex) => {
              ...
              return (
                <Card key={statIndex} className="border-2 border-[#ff5310] text-center">
                  <CardContent className="p-6">
                    <div className="bg-[#ff5310] p-3 rounded-full inline-block mb-4">
                      <IconComponent className="h-8 w-8 text-white" />
                    </div>
                    <div className="text-3xl font-bold text-[#ff5310] mb-2">{stat.value}</div>
